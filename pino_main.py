...
class PinoBot:
    ...
    def __init__(self):
        ...
        self.sleep = {
            ...
            "task_last_time": time.time(),
        }  # sec # last time when "Sleep_Mode_Task" occur
        ...
        self.wait = {
            ...
            "task_last_time": time.time(),
        }  # sec  # last time when "Wait_Mode_Task" occur

        self.keep_talk = {"max": 0, "cur_iter": 0}
    ...
    def main_loop_once(self):
        ...
        if self.detect["pre_state"] == 0 and cur_sensor_state == 1:
            ...
            if (
                self.sleep["state"]
                and time.time() - self.sleep["task_last_time"]
                > self.sleep["task_min_time"]
            ):
                ...
                time.time() - self.detect["first_time"] > self.sleep["enter_limit_time"]
            ):
        ...
        for task in self.reserved_task:
            ...
            if datetime.datetime.now().astimezone() > reserve_time:
                self.add_task(
                    task_type="event", event_name=task[1], event_parameter=task[2]
                )
    ...
    def add_task(
        ...
        self.log.info(
            "%s %s %s %r" % (task_type, event_name, event_parameter, fail_handler)
        )
        if isinstance(event_parameter, dict):
            ...
            try:
                ...
                self.log.warning(
                    "parse event parameter error, in future_event :%s" % event_parameter
                )
                parsed_parameter = None
            else:
                if isinstance(parsed_parameter, str):
                    self.log.warning(
                        "parse event parameter error:%s" % parsed_parameter
                    )
                    parsed_parameter = None
    ...
    def __run_task(self, task):
        ...
        if task_type == "talk":
            ...
            if self.cloud.gcloud_state < 0:
                if (
                    self.cloud.gcloud_state == -1 or -2
                ):  # 'Internet Error', 'google server error'
            ...
            # 2.E3. No matched Intent
                ...
                talk_responses[1].query_result.intent.display_name
        ...
        # 3. get "event" response
            ...
            # 2.E2. No matched Intent
                ...
                event_response.query_result.intent.display_name
                == "Default Fallback Intent"
            ):
                if fail_handler is not False:
                    event_response = self.cloud.send_event("FailNoMatch_Intent")
        ...
        for action_parameters in query_result["parameters"].keys():
            ...
            if len(check_cmd) != 1:
                ...
                try:
                    ...
                    self.log.warning(
                        "parse event parameter error, in future_event :%s"
                        % action_parameters
                    )
                    continue
                ...
                if isinstance(num, int) or isinstance(num, float):
                    ...
                    pino_commands.append(
                        [
                            num,
                            check_cmd[1],
                            query_result["parameters"][action_parameters].upper(),
                        ]
                    )
                    ...
                    dflow_parameters[action_parameters] = query_result["parameters"][
                        action_parameters
                    ].upper()
            else:
                dflow_parameters[action_parameters] = query_result["parameters"][
        ...
        if not is_pinoMotor:
            ...
            pino_commands.append(
                [0, "PinoMotor", str(rm)]
            )  # dflow parameter is str, so cast to str
        ...
        t1 = threading.Thread(
            target=self.__execute_pino_commands, args=(intent_name, pino_commands)
        )
        t1.start()
        ...
        if cmd_name.upper() == "PinoMotor".upper():
            ...
            try:
                ...
                if (
                    ...
                    or type(servo_time) is not float
                    or len(args) < 2
                ):
                    self.hardware.write(
                        text="DFlow Error \n %s \n PinoMotor" % intent_name,
    ...
    def __run_pino_event_commands(self, dflow_parameters):
        ...
        if (
            "PinoFutureEventName".upper() in cmd_list
            and "PinoFutureEventParameter".upper() in cmd_list
        ):
            event_time = datetime.datetime.strptime(
                dflow_parameters["time"], "%Y-%m-%dT%H:%M:%S%z"
            )  # "2020-08-06T18:00:00+09:00"
            event_name = dflow_parameters[
                "PinoFutureEventName"
            ]  #'2020-08-31T22:35:46+09:00'
            event_parameter = None
            if isinstance(dflow_parameters["PinoFutureEventParameter"], str):
                try:
                    event_parameter = ast.literal_eval(
                        dflow_parameters["PinoFutureEventParameter"].strip()
                    )
                except SyntaxError:
                    self.log.warning(
                        "parse event parameter error, in future_event :%s"
            ...
            self.log.info(
                "add reserved event %s %s %s %s"
                % (event_time, event_name, later, str(event_parameter))
            )
